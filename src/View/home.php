    ...
    <div class="franchise-grid">
        <?php foreach($franchisesList as $franchise): ?>
            <a class="franchise-card" href="<?= $basePath ?>/<?= htmlspecialchars($franchise->getSlug()) ?>">
                <img class="franchise-card__image <?php if(!$franchise->getActive()): ?> franchise-card--disabled<?php endif; ?>" src="<?= $basePath ?>/assets/img/games_icons/<?= htmlspecialchars($franchise->getImageUrl()) ?>" alt="<?= htmlspecialchars($franchise->getName()) ?>" loading="lazy">
